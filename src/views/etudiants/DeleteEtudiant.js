...
const DeleteEtudiant = () => {
    ...
    const SupprimerEtudiant = async(id) => {
          ...
          try {
            ...
            // const deleteEtudiant = await fetch(`http://localhost:5000/Etudiants/${id}`, {
            //     method: 'DELETE',
            // });
            // setEtudiant(etudiant.filter(etudiant => etudiant.id !== id));

            window.location = "/sample-page";
                    ...
                    <Form>
                        <Box mt="25px">
                            <Typography variant="body1" fontWeight={600} component="label" htmlFor="nom" mb="5px">
                                Êtes-vous sûr de vouloir supprimer l'étudiant <b>{etudiant.nom_etudiant} {etudiant.prenom_etudiant}</b> ?
                            </Typography>
                        </Box>
                    </Form>
                </Modal.Body>
